...
export default function EditTask({ close, id, idTask, titleTask, descTask, statusTask }:
    {
        close: () => void, id: number,
        idTask: number
        titleTask: string
        descTask: string
        statusTask: string
    ...
    return (
        <div className='w-screen h-screen fixed z-10 top-0 right-0'>
            ...
            <form onSubmit={(e) => {
                ...
                edite({ titulo: titulo.value, desc: desc.value, status: status.value })
            }}
                ...
                <h5 className="font-semibold text-2xl mb-2">Editar tarefa</h5>

                <label htmlFor="titulo" className="flex flex-col">
                    <span className="text-zinc-500 font-medium mb-1 text-sm">Título</span>
                    <input placeholder="Tarefa Exemplo" defaultValue={titleTask} type="text" name="titulo" id="titulo" className="border border-zinc-300 rounded-md px-2 py-1 outline-none" />
                </label>
                <label htmlFor="desc" className="flex flex-col">
                    <span className="text-zinc-500 font-medium mb-1 text-sm">Descricao</span>
                    <input placeholder="Descrição detalhada da tarefa." defaultValue={descTask} type="text" name="desc" id="desc" className="border border-zinc-300 rounded-md px-2 py-1 outline-none" />
                </label>
                <label htmlFor="status" className="flex flex-col">
                    <span className="text-zinc-500 font-medium mb-1 text-sm">Status</span>
                    <select defaultValue={statusTask} className="border border-zinc-300 rounded-md px-2 py-1 outline-none" name="status" id="status">
